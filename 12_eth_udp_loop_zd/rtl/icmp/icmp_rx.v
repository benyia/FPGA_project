...
// File name:           icmp_rx
// Created by:          正点原子
// Created date:        2025年10月22日10:56:00
// Version:             V1.0
// Descriptions:        以太网icmp数据接收模块
//
//----------------------------------------------------------------------------------------
//****************************************************************************************//
...
reg  [15:0]  icmp_rx_cnt        ; //接收ICMP数据计数
reg  [47:0]  des_mac_temp       ; //目的MAC地址
reg  [31:0]  des_ip_temp        ; //目的IP地址
...
//判断以太网的协议类型是不是ICMP
assign eth_type_match = (eth_type[15:8] == ETH_TYPE_IP[15:8]) && (gmii_rxd == ETH_TYPE_IP[7:0]);
...
always @(*) begin
    ...
    case(cur_state)
        ...
        st_ip_head : begin                    //接收IP首部
            if(gmii_rx_dv) begin
                if((rx_cnt == 5'd9) && (gmii_rxd != ICMP_TYPE))
                    next_state = st_rx_end;
                else if(rx_cnt == 5'd19)begin
                    if((des_ip_temp[23:0] == BOARD_IP[31:8]) && (gmii_rxd == BOARD_IP[7:0]))
        ...
        st_rx_data : begin                    //接收有效数据
            if(icmp_rx_cnt == icmp_data_length - 16'd1)
                next_state = st_rx_end;
            else
                next_state = st_rx_data;
        end
        ...
        case(cur_state)
            ...
            st_ip_head : begin
                if(gmii_rx_dv) begin
                    ...
                    else if((rx_cnt >= 5'd16) && (rx_cnt <= 5'd19))
                        des_ip_temp <= {des_ip_temp[23:0],gmii_rxd};   //目的IP地址
                    else;
                end
                else ;
            end
