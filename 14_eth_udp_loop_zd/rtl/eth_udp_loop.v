...
// Created by:          正点原子
// Created date:        2025年10月22日14:12:00
// Version:             V1.0
...
// ILA例化（无专门触发端口，用探针信号触发）
ila_0 u_ila_0 (
    .clk                 (sys_clk),      // ILA时钟（建议用gmii_rx_clk或sys_clk）
    .probe0              (gmii_rx_dv),   // 探针0：GMII接收有效
    .probe1              (gmii_rxd),      // 探针1：GMII接收数据[7:0]
    .probe2              (gmii_tx_en),    // 探针2：GMII发送有效
    .probe3              (gmii_txd),      // 探针3：GMII发送数据[7:0]
    .probe4              (rec_pkt_done),  // 探针4：UDP接收完成
    .probe5              (rec_en),        // 探针5：UDP接收使能
    .probe6              (rec_data),      // 探针6：UDP接收数据[7:0]
    .probe7              (tx_start_en),   // 探针7：UDP发送开始
    .probe8              (tx_data),       // 探针8：UDP发送数据[7:0]
    .probe9              (udp_tx_done)    // 探针9：UDP发送完成
    // 注意：这里没有trig0端口！
);
endmodule