...
module udp_tx(    
    ...
    output  reg         udp_tx_busy   //debug   高电平，busy
    );
...
localparam  udp_tx_idle      = 7'b000_0001; //初始状态，等待开始发送信号
localparam  udp_tx_check_sum = 7'b000_0010; //IP首部校验和
localparam  udp_tx_preamble  = 7'b000_0100; //发送前导码+帧起始界定符
localparam  udp_tx_eth_head  = 7'b000_1000; //发送以太网帧头
localparam  udp_tx_ip_head   = 7'b001_0000; //发送IP首部+UDP首部
localparam  udp_tx_tx_data   = 7'b010_0000; //发送数据
localparam  udp_tx_crc       = 7'b100_0000; //发送CRC校验值

localparam  ETH_TYPE     = 16'h0800  ;  //以太网IP协议
...
always @(*) begin
    ...
    case(cur_state)
        udp_tx_idle     : begin                               //等待发送数据
            if(skip_en)                
                next_state = udp_tx_check_sum;
            else
                next_state = udp_tx_idle;
        end  
        udp_tx_check_sum: begin                               //IP首部校验
            if(skip_en)
                next_state = udp_tx_preamble;
            else
                next_state = udp_tx_check_sum;    
        end                             
        udp_tx_preamble : begin                               //发送前导码+帧起始界定符
            if(skip_en)
                next_state = udp_tx_eth_head;
            else
                next_state = udp_tx_preamble;      
        end
        udp_tx_eth_head : begin                               //发送以太网首部
            if(skip_en)
                next_state = udp_tx_ip_head;
            else
                next_state = udp_tx_eth_head;      
        end              
        udp_tx_ip_head : begin                                //发送IP首部+UDP首部               
            if(skip_en)
                next_state = udp_tx_tx_data;
            else
                next_state = udp_tx_ip_head;      
        end
        udp_tx_tx_data : begin                                //发送数据                  
            if(skip_en)
        ...
        case(next_state)
            udp_tx_idle     : begin
                ...
                if(trig_tx_en) begin
                    ...
                    ip_head[5] <= {BOARD_UDP_PORT,DES_UDP_PORT};//{16'd1234,16'd1234};  
                    //16位udp长度，16位udp校验和              
